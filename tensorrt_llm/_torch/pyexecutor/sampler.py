...
class TorchSampler(Sampler):
    ...
    def handle_logprobs(
        ...
        *,
        ...
        if request.py_return_log_probs:
            ...
            # Follow vLLM/OpenAI API behavior:
            # - py_num_logprobs == 0: Return only sampled token's logprob
            # - py_num_logprobs > 0: Return sampled token's logprob (first) + top-K logprobs (up to K+1 elements)
            if request.py_num_logprobs == 0:
                ...
                # Return sampled token first + top-K logprobs (py_num_logprobs > 0)
                # This matches vLLM/OpenAI behavior where sampled token is always included
                sampled_logprobs = request.py_sampled_logprobs[:count]
                ...
                for step, (sampled_token, sampled_logprob, topk_tokens, topk_logprobs) in enumerate(
                    ...
                    step_dict = {}
                    # Always add sampled token first
                    step_dict[sampled_token] = Logprob(logprob=sampled_logprob.item(), rank=1)
                    
                    # Add top-K tokens (skip if already added as sampled token to avoid duplicates)
                    for rank, (token, logprob) in enumerate(
                        zip(topk_tokens.tolist(), topk_logprobs.tolist())
                    ):
                        if token not in step_dict:
                            step_dict[token] = Logprob(logprob=logprob, rank=rank + 1)
    ...
    def _process_requests(
        ...
        *,
        ...
        # Handle logprobs computation. This is done outside the sampling loop,
        ...
        if return_log_probs:
            ...
            # Separate requests: those wanting topk (py_num_logprobs > 0) vs sampled only (py_num_logprobs == 0)
            topk_logprobs_req_indices = [
                req_id for req_id, req in enumerate(requests) if req.py_num_logprobs and req.py_num_logprobs > 0
            ]
            sampled_only_req_indices = [
                req_id for req_id, req in enumerate(requests) if req.py_num_logprobs == 0 and req.py_return_log_probs
            ]
            
            # Combine all requests that need logprob computation
            all_logprobs_req_indices = topk_logprobs_req_indices + sampled_only_req_indices
            
            if all_logprobs_req_indices:
                logprobs_logit_indices = logits_cuda_indexer[all_logprobs_req_indices]
            ...
            # Handle top-k logprobs for requests with py_num_logprobs > 0
            if topk_logprobs_req_indices:
                topk_vals_cuda, topk_indices_cuda = torch.topk(
                    logprobs_cuda[:len(topk_logprobs_req_indices)], 
                    k=max(req.py_num_logprobs for req in requests if req.py_num_logprobs and req.py_num_logprobs > 0), 
                    dim=-1
                )
                ...
                for req_id, steps in zip(
                    topk_logprobs_req_indices, req_num_steps[topk_logprobs_req_indices].tolist()
                ):
                    req = requests[req_id]
                    next_offset = current_offset + steps
        ...
        if return_log_probs and all_logprobs_req_indices:
            sampled_tokens_cuda = batched_sampling_result.batch_next_tokens_cuda_int
            logprobs_req_set = set(all_logprobs_req_indices)
            sampled_logprobs_list = []

            for req_id in range(len(requests)):
                if req_id in logprobs_req_set:
                    logprobs_idx = all_logprobs_req_indices.index(req_id)

                    if logprobs_idx == 0:
                        start_offset = 0
                    else:
                        start_offset = sum(req_num_steps[all_logprobs_req_indices[:logprobs_idx]].tolist())
